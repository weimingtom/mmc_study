...
# include "../libevent-1.4.4-stable-win32/event.h"
#include <stdio.h>
#include <ws2tcpip.h>
#include <assert.h>
#define writev(socket, buf, len) write(socket, buf, len)
#else /* !WIN32 */
#include <sys/types.h>
...
void libmemc_destroy(struct Memcache* handle) {
    int ii;
	for (ii = 0; ii < handle->no_servers; ++ii) {
...
int libmemc_add_server(struct Memcache *handle, const char *host, in_port_t port) {
    ...
    server = server_create(host, port);
    if (server != NULL) {
...
static int libmemc_store(struct Memcache* handle, enum StoreCommand cmd,
    ...
    if (server == NULL) {
        ...
        if (server->sock == -1) {
			printf("libmemc_store server_connect start\n");
            if (server_connect(server) == -1) {
                return -1;
            }
        }
		printf("libmemc_store server_connect store\n");
        if (handle->protocol == Binary) {
...
static int server_sendv(struct Server* server, struct iovec *iov, int iovcnt) {
    ...
    // @todo I might have a scattered IO function on windows...
	{
		...
		for (ii = 0; ii < iovcnt; ++ii) {
			if (send(server->sock, iov[ii].iov_base, iov[ii].iov_len, 0) != 0) {
				return -1;
			}
		}
	}
...
static int textual_get(struct Server* server, struct Item* item) {
    ...
    nread = server_receive(server, server->buffer,server->buffersize, 1);
    ...
    if (strstr(server->buffer, "VALUE ") == server->buffer) {
        ...
        headsize = ptr - server->buffer;
        chunk = nread - headsize;
